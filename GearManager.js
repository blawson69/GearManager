...
var GearManager = GearManager || (function () {
    ...
    var version = '0.1',
        ...
        gear = [
            ...
            {name: "Antitoxin", shaped_script: true, section: "utility", category: "Adventuring Gear", content: "A creature that drinks this vial of liquid gains advantage on Saving Throws against poison for 1 hour. It confers no benefit to Undead or constructs."},
            ...
            {name: "Dust of Disappearance", shaped_script: false, section: "utility", category: "Wondrous Items", content: "Found in a small packet, this powder resembles very fine sand. There is enough of it for one use. When you use an action to throw the dust into the air, you and each creature and object within 10 feet of you become Invisible for 2d4 minutes. The Duration is the same for all subjects, and the dust is consumed when its magic takes effect. If a creature affected by the dust attacks or casts a spell, the Invisibility ends for that creature."},

            {name: "Dust of Dryness", shaped_script: false, section: "utility", category: "Wondrous Items", content: "This small packet contains 1d6 + 4 pinches of dust. You can use an action to sprinkle a pinch of it over water. The dust turns a cube of water 15 feet on a side into one marble-sized pellet, which floats or rests near where the dust was sprinkled. The pellet's weight is negligible.\nSomeone can use an action to smash the pellet against a hard surface, causing the pellet to Shatter and release the water the dust absorbed. Doing so ends that pellet's magic.\nAn elemental composed mostly of water that is exposed to a pinch of the dust must make a DC 13 Constitution saving throw, taking 10d6 necrotic damage on a failed save, or half as much damage on a successful one.", uses: "1d6+4"},
            ...
            {name: "Gem of Seeing", shaped_script: false, section: "utility", category: "Wondrous Items", content: "This gem has 3 Charges. As an action, you can speak the gem's Command Word and expend 1 charge. For the next 10 minutes, you have Truesight out to 120 feet when you peer through the gem.\nThe gem regains 1d3 expended Charges daily at dawn.", uses: 3},

            {name: "Helm of Teleportation", shaped_script: false, section: "utility", category: "Wondrous Items", content: "This helm has 3 Charges. While wearing it, you can use an action and expend 1 charge to cast the Teleport spell from it. The Helm regains 1d3 expended Charges daily at dawn.", uses: 3},
    ...
    commandList = function() {
        ...
        _.each(ag_items, function(item) {
            uses = (item.uses && typeof item.uses === 'string') ? ' &#63;&#123;Uses&#124;[[' + item.uses + ']]&#125;' : '';
        ...
        _.each(wi_items, function(item) {
            uses = (item.uses && typeof item.uses === 'string') ? ' &#124;&#63;&#123;Uses&#124;' + rollDice(item.uses) + '&#125;' : '';
            //if (item.uses && typeof item.uses === 'string') rollDice(item.uses);
            //log('Is ' + ((item.uses) ? item.uses : 'undefined') + ' a string? ' + (item.uses && typeof item.uses === 'string'));
            list += '<tr><td style="width: 100%"><a href="!gm --add ' + item.name + uses + '">' + item.name + '</a></td><td><a href="!gm --view '
            + item.name + '">view</a></td></tr>';
        });
    ...
    commandAdd = function(msg) {
        ...
        if (item) {
            ...
            if (charNames.length > 0) {
                ...
                showDialog('Gear Added', 'A <b>' + item.name + '</b> was given to ' + charNames.join(joiner) + '.');
            } else {
                showDialog('Gear Not Added', 'The selected characters already possess <b>' + item.name + '</b>. Nothing was added.');
            }
        } else {
            showDialog('Gear Not Added', 'The item <b>"' + tmpName + '"</b> does not exist! Try adding from the list this time.<div style="'
            + styles.fullWidth + '"><a style="' + styles.button + '" href="!gm --list">&#9668; Back to List</a></div>');
        }
    },
    ...
    rollDice = function (expression) {
        ...
        var dice = parseInt(expression.split('d')[0]);
        var die = parseInt(expression.split('d')[1].split('+')[0]);
        var bonus = (expression.match(/\+/g) !== null) ? parseInt(expression.split('+')[1]) : 0;
        var result = bonus;
